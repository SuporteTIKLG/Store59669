      ...
      smallScreenFlag = 1,
		...
		/*Show the Offcanvas*/
                nav.toggleOffcanvas = function(){
                    if(!nav.classList.contains(nav.options.landscapeClass)){
                        if(!nav.navigationBody.classList.contains("is-visible")){
                            nav.navigationBody.classList.add("is-visible");
                            ...
                            nav.navigationBody.classList.add("is-invisible");
                            if(!nav.navigationBody.check("transitionend")){
                                nav.navigationBody.on("transitionend", function(){
                                    nav.navigationBody.classList.remove("is-visible", "is-invisible");
                                    nav.navigationBody.off("transitionend");
                                });
                            }
                            nav.overlayPanel.classList.add("is-invisible");
                            if(!nav.overlayPanel.check("transitionend")){
                                nav.overlayPanel.on("transitionend", function(){
                                    nav.overlayPanel.classList.remove("is-visible", "is-invisible");

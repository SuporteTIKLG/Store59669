...
var fcSpy$=(function(){
  ...
  function removeLastSmartModal(){
      ...
      var modalArea = document.querySelector('.sModalArea');
      if(modalArea){
          modalArea.style.transform = 'translateX(100%)';
      }
      window.history.replaceState("object or string","Title",actualPage);
      document.title = actualTitle;
      var canonicalLink = window.top.document.querySelector("link[rel='canonical']");
      if (canonicalLink) {
          canonicalLink.href = actualCanonical;
      }
      document.body.removeAttribute('style');
      var getNodeToRemove = document.getElementById('sModalSpy');
      if(getNodeToRemove && getNodeToRemove.parentNode){
          setTimeout(function () {

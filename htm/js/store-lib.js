...
var sF$=(function(){
  ...
  function fnShowProd(Price,OriginalPrice,Cod,iMaxParcels,ProductID,bMinPrice){
    ...
    if(Price!=OriginalPrice && !bMinPrice){
      ...
      sPrice+="  <div class=\"old-price font-regular\">"+ rk("list-price-was") +"&nbsp; "+ FCLib$.formatMoney(OriginalPrice,FC$.Currency) +"</div>";
      sPrice+="  <div class=\"price home-price font-bold\">"+ rk("list-price-now") +" "+ FC$.Currency + " " + FCLib$.formatMoneyInt(iPriceInt) + FCLib$.getDecimalSep() + PriceDecimal +"</div>";
      if(iMaxParcels>1)sPrice+="  <div class=\"installments font-regular\">"+ iMaxParcels +"x "+ rk("payment-table-installments-of") +" "+ FCLib$.formatMoney(CalculateInstallment(Price,iMaxParcels),FC$.Currency) +""+ sInterest +"</div>";
      sPrice+="</div>";
    }
    else{
      sPrice+="<div class=\"prices\">";
      if(bMinPrice)sPrice+="  <div class=\"old-price font-regular\">"+ rk("product-list-from") +"&nbsp;</div>";
      sPrice+="  <div class=\"price home-price font-bold\">"+ FC$.Currency + " " + FCLib$.formatMoneyInt(iPriceInt) + FCLib$.getDecimalSep() + PriceDecimal +"</div>";
      if(iMaxParcels>1)sPrice+="  <div class=\"installments font-regular\">"+ iMaxParcels +"x "+ rk("payment-table-installments-of") +" "+ FCLib$.formatMoney(CalculateInstallment(Price,iMaxParcels),FC$.Currency) +""+ sInterest +"</div>";
      sPrice+="</div>";
    }
      ...
      for(var i=0;i<aImgsShare.length;i++){
        ...
        if(aImgsShare[i].className=='EstImgShareFacebook'){
          if(fnGetConfig("Facebook_Page_Link")){
            aImgsShare[i].setAttribute(sSrc,FC$.PathImg +'iconprodfacebook.svg'+ '');
          ...
          if(fnGetConfig("Twitter_Page_Link")){
            aImgsShare[i].setAttribute(sSrc,FC$.PathImg +'iconprodtwitter-x.svg'+ '');
          ...
          if(fnGetConfig("Instagram_Page_Link")){
            aImgsShare[i].setAttribute(sSrc,FC$.PathImg +'iconprodinstagram.svg'+ '');
          ...
          if(fnGetConfig("Youtube_Page_Link")){
            aImgsShare[i].setAttribute(sSrc,FC$.PathImg +'iconprodyoutube.svg'+ '');
        ...
        else if(aImgsShare[i].className=='EstImgShareTikTok'){
          if(fnGetConfig("TikTok_Page_Link")){
            aImgsShare[i].setAttribute(sSrc,FC$.PathImg +'iconprodtiktok.svg'+ '');
            aImgsShare[i].setAttribute("alt","TikTok");
            if(FC$.LazyLoad>0)aImgsShare[i].setAttribute("loading","lazy");
            aImgsShare[i].onclick=function(){window.open('https://www.tiktok.com/'+ fnGetConfig("TikTok_Page_Link"));};
            aImgsShare[i].style.cursor="pointer";aImgsShare[i].style.margin="0 5px";
          } else {aImgsShare[i].style.display="none";}
        }
    }
  ...
  function fnLoginUserName(NameUser,PicUser){
    ...
    if(NameUser==""){
      document.getElementById("fc-loginInfo").innerHTML="<a href='"+ FCLib$.uk("url-account") +"'><span class=HeaderSocialLoginLogout></span></a>";
      if(oImgGlobalSign){oImgGlobalSign.style.display="";}
    }
    else{
      NameUser=fnFirstName(NameUser);
      document.getElementById('fc-loginInfo').innerHTML = "<a href='#Logout' onclick=FCLib$.fnClientLogout('',sF$.fnCliLogout)><span class=HeaderSocialLoginLogoutOff></span></a>";
  ...
  function fnSlideshowStore(aBanners){
    ...
    for(i=0;i<numBannerHomeFC && i<aBanners.length;i++){
      oBanner=aBanners[i];
      oSlideshow.innerHTML+='<div class="carousel-banner"><a href="/banner-redir.ehc?idbanner='+ oBanner.bannerID +'&url='+ encodeURIComponent(oBanner.url) +'"><picture><source media="(max-width:767px)" srcset="'+ oBanner.imageMob +'"><img alt="'+ oBanner.alt.replace(/"/g,'&quot;')+'" src="'+ oBanner.image+ '" width="1900" height="350"></picture><div class="container-banner-text"></div></a></div>';
    }
    FCLib$.execLazyLoad();
  }
  ...
  function fnInsertAltShimImg(){
    if(FC$.Page=="Products"){ 
      ...
      for(i=0;i<insertAltShim.length;i++){
        ...
        insertAltShim[i].setAttribute("width","11");
        insertAltShim[i].setAttribute("height","14");
      }                   
    }
  }  
  ...
  function fnFloatingWhatsApp(){
    if(fnGetConfig("Show_WhatsApp")){
      ...
      createDivFloatingWhatsApp.innerHTML = '<a href="https://api.whatsapp.com/send?phone='+ fnGetConfig('Phone_WhatsApp') +'&text='+ encodeURIComponent(rk("whatsapp-text")) +'" target="_blank" rel="noopener"><img alt="Whatsapp" width="50" height="50" src="'+ FC$.PathImg +'footer-sticky-whatsapp.svg'+ '"></a>';
  ...
  function fnInsertDynCarouselCrossSelling(){
    if(fnGetConfig("Carousel_Cross_Selling")){
      ...
      if(elementCrossSellingID){
        ...
        document.querySelectorAll("#idListProdCrossFC .EstFotoProdCross").forEach(oImg=>{var sImg=oImg.getAttribute("data-src");oImg.removeAttribute("data-src");oImg.setAttribute("width","152");oImg.setAttribute("height","152");if(FC$.LazyLoad==2)oImg.setAttribute("data-flickity-lazyload",sImg);});
  ...
  function fnHeaderDropdown(){
    ...
    var useFullMenu=window.innerWidth<1024;
    var menuToUse=useFullMenu ? FCLib$.oCatMenu : FCLib$.oCatMenu.slice(0,8);
    function renderSubMenu(subMenu){
      var textToInsert='<ul class="navigation-dropdown navigation-submenu 0123456 ">';
      for(var iii in subMenu){
        textToInsert+='<li class="navigation-dropdown-item">';
        textToInsert+='<a href="'+ subMenu[iii].url +'" class="navigation-dropdown-link">'+ subMenu[iii].name +'</a>';
        if(subMenu[iii].subs){
            textToInsert+=renderSubMenu(subMenu[iii].subs);
        }
        textToInsert+='</li>';
      }
      textToInsert+='</ul>';
    ...
    for(var i in menuToUse){
      var textToInsert='<li class="navigation-item">';
      textToInsert+='<a href="'+ menuToUse[i].url +'" class="navigation-link">'
      textToInsert+='<div class="navigation-link-url">'+ menuToUse[i].name +'</div></a>';
      if(menuToUse[i].subs){
        textToInsert+='<ul class="navigation-dropdown navigation-submenu navigation-submenu-arrow" style="right:auto;">';
        for(var ii in menuToUse[i].subs){
          textToInsert+='<li class="navigation-dropdown-item">';
          textToInsert+='<a href="'+ menuToUse[i].subs[ii].url +'" class="navigation-dropdown-link">'+ menuToUse[i].subs[ii].name +'</a>';
          if(menuToUse[i].subs[ii].subs){
            textToInsert+=renderSubMenu(menuToUse[i].subs[ii].subs);
          }
          textToInsert+='</li>';
        }
        textToInsert+='</ul>';
  ...
  /* Web Share API - Product Details */
  function fnWebShareAPIProduct(storeName,prodName,prodStoreID,prodID){
    var shareAPI = document.querySelector('#webshare-product');
    shareAPI.addEventListener('click',async event=>{
      if(navigator.share){
        try{
          await navigator.share({
            title:`${storeName}`,
            text:`${prodName}`,
            url:`http://lvfc.co/l${prodStoreID}/p${prodID}`
          });
        }catch(error){
          console.error(`Error while using Web Share API:\n${error}`);
        }
      }
    });
  }
  
  /* Load Smart Modal Product List */
  function fnLoadSmartModalProductList(){
    var bodyClasses=document.body.className;
    if(bodyClasses.includes("ProductList")){
      var scriptTag=document.createElement("script");
      scriptTag.src=FC$.PathHtm +"js/smartmodal-min.js";
      scriptTag.defer=true;
      document.head.appendChild(scriptTag);
    }
  }
  
  /* Loading Icon Width and Height */
  function fnSetHeightWidthLoading(){
    var iconLoading=document.getElementsByClassName("EstLoadingProdFC");
    var iconHeight=35;
    var iconWidth=35;
    for(var i=0;i<iconLoading.length;i++){
      iconLoading[i].style.height=iconHeight +"px";
      iconLoading[i].style.width=iconWidth +"px";
    }
  }
  
  /* Remove image link */
  function fnRemoveImageLink(prodID,URLProd){
    function removeImgLink(){
      document.getElementById("DivImagemProdDouble"+prodID).addEventListener("click",function(event){
        event.preventDefault();
      });
    }
    if(!sF$.fniOSversion() && !sF$.fnMacOsSafariversion()){
      removeImgLink();
    }else if(sF$.fniOSversion() && sF$.fniOSversion()[0] >= 13){
      removeImgLink();
    }else if(sF$.fnMacOsSafariversion() && sF$.fnMacOsSafariversion()[0] > 13){
      removeImgLink();
    }
    if((sF$.fniOSversion() && sF$.fniOSversion()[0] < 13) || (sF$.fnMacOsSafariversion() && sF$.fnMacOsSafariversion()[0] < 13)){
      sF$.fnRemoveSpy2From("DivImagemProdDouble"+prodID,URLProd);
      sF$.fnRemoveSpy2From("prod-list-container-grid-view",URLProd);
      sF$.fnRemoveSpy2From("DivHomeDetails",URLProd);
      sF$.fnRemoveSpy2From("DivHomeNomeProd<prodId>","<URLProd>");
      sF$.fnRemoveSpy2From("DivHomePriceProd<prodId>","<URLProd>");
      sF$.fnRemoveSpy2From("DivProductListNomeProd<prodId>","<URLProd>");
      sF$.fnRemoveSpy2From("DivProductListPriceProd<prodId>","<URLProd>");
      sF$.fnRemoveSpy2From("DivProdListDetails","<URLProd>");
    }
  }
  
  /* Product Accordion Installment */
  function fnProductMaxInstallment(prodMaxInstallmentNum){
    function fnDefineMaxInstallment(){
      let maxInstallmentNum = prodMaxInstallmentNum;
      if(typeof ParcelsValues != "undefined" && maxInstallmentNum == 0){
        let iPosArrayValue = 0;
        for(var i=0;i<ParcelsValues.length && TPProdPrice>=ParcelsValues[i];i++){
          iPosArrayValue = i;
        }
        maxInstallmentNum = iPosArrayValue + 1;
      }
      if(typeof ParcelsInterests != "undefined"){
        if(maxInstallmentNum == 0 || maxInstallmentNum > ParcelsInterests.length) maxInstallmentNum = ParcelsInterests.length;
      }else maxInstallmentNum = 0;
      return maxInstallmentNum;
    }
    function fnDisplayCard(maxInstallmentNum){
      let TPMainDIV = document.querySelector(".panelFC");
      let TPMainDIVHTML = `
        <div class="TPiconesCartoes">
          <ul>
            <li><img width="38" height="39" alt="MasterCard" src="${FC$.PathImg}/fc-mastercard-2.svg"></li>
            <li><img width="38" height="39" alt="Visa" src="${FC$.PathImg}/fc-visa.svg"></li>
            <li><img width="38" height="39" alt="Amex" src="${FC$.PathImg}/fc-amex.svg"></li>
            <li class="TPDisplayIconsCards"><img width="38" height="39" alt="Elo" src="${FC$.PathImg}/fc-elo.svg"></li>
          </ul>
        </div>
        <div class="TPtabelaParcelas">
          <ul class="TPtabelaParcelasUL">`;
      for(let i=0;i< maxInstallmentNum;i++){
        TPMainDIVHTML += `<li><b>` + (i + 1) + `<var key="payment-table-installments-x"></var></b> <var key="payment-table-installments-of"></var> ` + FCLib$.formatMoney(CalculateInstallment(TPProdPrice, (i + 1)), FC$.Currency) + `<var key="payment-table-installments-month"></var></li>`;
      }
      TPMainDIVHTML += `
          </ul>
        </div>`;
      TPMainDIV.innerHTML=TPMainDIVHTML;
      var paymentTableAccordion = document.getElementsByClassName("accordionFC");
      for(i=0;i<paymentTableAccordion.length;i++){
        paymentTableAccordion[i].addEventListener("click",function(){
          this.classList.toggle("activeFC");
          var paymentTablePanel=this.nextElementSibling;
          if(paymentTablePanel.style.maxHeight){
            paymentTablePanel.style.maxHeight=null;
          }else{
            paymentTablePanel.style.maxHeight=paymentTablePanel.scrollHeight +"px";
          }
        });
      }
    }
    let iProdMaxInstallment=fnDefineMaxInstallment();
    if(iProdMaxInstallment>1)fnDisplayCard(iProdMaxInstallment);
    else document.getElementById("tpDisplayCard").style.display="none";
  }
  
  /* FastPreview Change URL */
  function fnFastPreviewChangeURL(){
    var chkURLTop=((location.pathname).indexOf("prod,sty,5,int,1,") > -1),
        bFromSpy=(document.referrer.indexOf("sty,5") > -1 || document.referrer.indexOf("sty=5") > -1),
        urlToShow;
    if(!chkURLTop){
      urlToShow=location.pathname + location.search;
      if(urlToShow.indexOf(",sty,5,int,1") > -1)urlToShow=urlToShow.replace(",sty,5,int,1","");
      if(urlToShow.indexOf("&sty=5&int=1") > -1)urlToShow=urlToShow.replace("&sty=5&int=1","");
      if(urlToShow.indexOf("?sty=5&int=1") > -1)urlToShow=urlToShow.replace("?sty=5&int=1","");
    }else{
      urlToShow=(location.pathname).replace("prod,sty,5,int,1,","prod,");
    }
    if(bFromSpy){
      window.top.history.replaceState("object or string","Title",urlToShow);
    }else{
      window.top.history.pushState("object or string","Title",urlToShow);
    }
    var swipperArea=document.querySelector(".ProductDet");
    fcSpy$.swipedetect(swipperArea,function(swipedir){
      if(swipedir=="right")window.top.fcSpy$.removeLastSmartModal();
    });
    if(!bFromSpy){
      FCLib$.AddEvent(window.top,"popstate",function(){
        setTimeout(function(){
          window.top.fcSpy$.removeLastSmartModal();
        },1);
      });
    }
  ...
  /* Cross Selling open on iframe */
  function fnChangeLinksCrossSelOnFCSpy(ElementWithLinksToChange){
    var getCSlinks=Array.prototype.slice.call(document.querySelectorAll(ElementWithLinksToChange+' a'));
    var regex=/(prod,)(.*)/;
    getCSlinks.forEach(function(item){
      if(item.href.indexOf("prod,")!=-1){
        item.href=item.href.replace(regex, "$1sty,5,int,1,$2");
      }else if(item.href.indexOf("?")!=-1){
        item.href+="&sty=5&int=1";
      }else{
          item.href+="?sty=5&int=1";
        }
    });
    var getCSDIVs=Array.prototype.slice.call(document.querySelectorAll(ElementWithLinksToChange +" div"));
    getCSDIVs.forEach(function(item){item.removeAttribute("onclick");});
    var getLinkToMainDiv=Array.prototype.slice.call(document.querySelectorAll(".EstNameProdCross a"));
    getLinkToMainDiv.forEach(function(item){(item.parentElement.parentElement).setAttribute("onclick","location.href='"+item.href+"';");});
  };
  
  /* Comments on a top window */
  function fnChangeTargetLinksOnFCSpy(ElementWithLinksToChange){
    var getCSlinks=Array.prototype.slice.call(document.querySelectorAll(ElementWithLinksToChange +" a"));
    getCSlinks.forEach(function(item){
  ...
  function fnAdjustButtonLocation(){
    function buttonLocation(){
      var btcomprar=document.getElementById("idButtonBuyFC_1");
      var getFrameFromInside=window.top.document.getElementById("idIframe");
      if(btcomprar && getFrameFromInside){
        btcomprar.className += " movedButton";
        (getFrameFromInside.parentNode.parentNode).appendChild(btcomprar);
      }
    }
    function iOSversion(){
      if(/iP(hone|od|ad)/.test(navigator.userAgent)){
    ...
    if(window.innerWidth <= 1023 && iOSversion() && iOSversion()[0] < 13){
      setTimeout(function(){
        buttonLocation();
      }, 200);
      document.getElementsByTagName("HEAD")[0].innerHTML +="\n<style>" +
        ".mz-expand .mz-image-stage > figure:before, .mz-expand .mz-image-stage:before{height:20%!important;}" +
        ".mz-expand-thumbnails{top:17%!important;bottom: auto !important;}" +
        "</style>";
    }
  }

  /* Product List Filter View More or Less*/
  function fnFilterViewMore(){
    class FilItemsList{
      constructor(listElement){
        ...
        this.showMoreLink.textContent=rk("prod-list-filter-button-show-more");
        this.showMoreLink.classList.add("button-ShowMore");
        this.showMoreLink.addEventListener("click",this.toggleVisibility.bind(this));
        this.listElement.appendChild(this.showMoreLink);
        this.toggleItemsVisibility();
      }
      toggleVisibility(event){
        event.preventDefault();
        for (i=7;i<this.listItems.length;i++){
          this.listItems[i].style.display=(this.listItems[i].style.display==="none")?"":"none";
        }
        if(this.showMoreLink.textContent===rk("prod-list-filter-button-show-more")){
          this.showMoreLink.textContent=rk("prod-list-filter-button-show-less");
          this.showMoreLink.classList.remove("button-ShowMore");
          this.showMoreLink.classList.add("button-ShowLess");
        }else{
          this.showMoreLink.textContent=rk("prod-list-filter-button-show-more");
          this.showMoreLink.classList.remove("button-ShowLess");
          this.showMoreLink.classList.add("button-ShowMore");
        }
      }
      toggleItemsVisibility(){
        if(this.listItems.length >= 8){
          ...
          this.showMoreLink.style.display="inline";
        }else{
          this.showMoreLink.style.display="none";
        }
      }
    }
    const bodyHasClass=document.body.classList.contains("FCProduct") && document.body.classList.contains("ProductList");
    if(bodyHasClass){
      const listElements = document.querySelectorAll(".FilItemsList");
      listElements.forEach(element => new FilItemsList(element));
    }
  }
  
  /* Fixed Filter Bar Product List */
  function fnFixDIVsSizes(){
    if(innerWidth>1022 && (document.body.className==='FCProduct ProductList' || document.body.className==='FCCustom HighlightsPage')){
      var getProductsFilterFC,getContentFil,getTabTitProd;
      FCLib$.onLoaded(function(){
        getTabTitProd=document.getElementById("TabTitProd");
        if(FC$.Page==='Custom'){
          getProductsFilterFC=document.getElementById("ProductsFilterFC_Hotsite");
        }else{
          getProductsFilterFC=document.getElementById("ProductsFilterFC");
        }
        getContentFil=document.getElementById("ContentFil");
        if(getProductsFilterFC && getContentFil){
          function updateSizes(){
            var prodsDIVSizeAjusted=getTabTitProd.offsetHeight - 100;
            var prodsDIVSizeAjustedCHighlights=getTabTitProd.offsetHeight - 100;
            if(getProductsFilterFC)getProductsFilterFC.style.minHeight=prodsDIVSizeAjusted +"px";
            if(getContentFil)getContentFil.style.height=(innerHeight - 120) +"px";
            if(getContentFil)getContentFil.style.maxHeight=prodsDIVSizeAjusted +"px";
          }
          updateSizes();
          window.addEventListener('resize',updateSizes,true);
  ...
  /* Harden Site Scrutiny */
  function fnDisableCode(){
    if(fnGetConfig("Harden_Site_Scrutiny")){
      ...
      var enableInspection=getUrlParameter("code");
      if(enableInspection==="true")FCLib$.SetCookie("enableInspection","true");
      else if(enableInspection==="false")FCLib$.SetCookie("enableInspection","false");
      else enableInspection=FCLib$.GetCookie("enableInspection");
      if(enableInspection!=="true"){
        document.addEventListener('contextmenu',function(e){e.preventDefault();}); /* Disable right click */
        document.addEventListener("keydown",function(e){ /* Disable key combinations */
          if(e.keyCode==123){e.preventDefault();return false;} /* Disable F12 (Developer Tools) */
          if(e.ctrlKey && e.shiftKey && e.keyCode=="I".charCodeAt(0)){e.preventDefault();return false;} /* Disable Ctrl+Shift+I (Developer Tools) */
          if(e.ctrlKey && e.shiftKey && e.keyCode=="J".charCodeAt(0)){e.preventDefault();return false;} /* Disable Ctrl+Shift+J (Console) */
          if(e.ctrlKey && e.keyCode=="U".charCodeAt(0)){e.preventDefault();return false;} /* Disable Ctrl+U (View Source) */
          if(e.ctrlKey && e.shiftKey && e.keyCode=="C".charCodeAt(0)){e.preventDefault();return false;} /* Disable Ctrl+Shift+C (Inspect Element) */
          if(e.ctrlKey && e.shiftKey && e.keyCode=="K".charCodeAt(0)){e.preventDefault();return false;} /* Disable Ctrl+Shift+K (Console in Firefox) */
          if(e.ctrlKey && e.keyCode=="S".charCodeAt(0)){e.preventDefault();return false;} /* Disable Ctrl+S (Save) */
          if(e.ctrlKey && e.keyCode=="A".charCodeAt(0)){e.preventDefault();return false;} /* Disable Ctrl+A (Select all) */
          if(e.ctrlKey && e.shiftKey && e.keyCode=="S".charCodeAt(0)){e.preventDefault();return false;} /* Disable Ctrl+Shift+S (Save As) */
        });
        /* Disable mouse and touch selection */
        ...
        document.addEventListener("touchmove",function(e){if(e.touches.length>1){e.preventDefault();}});
  ...
  return{
    ...
    fnWebShareAPIProduct:fnWebShareAPIProduct,
    fnLoadSmartModalProductList:fnLoadSmartModalProductList,
    fnSetHeightWidthLoading:fnSetHeightWidthLoading,
    fnRemoveImageLink:fnRemoveImageLink,
    fnProductMaxInstallment:fnProductMaxInstallment,
    fnFastPreviewChangeURL:fnFastPreviewChangeURL,
    fnChangeLinksCrossSelOnFCSpy:fnChangeLinksCrossSelOnFCSpy,
    fnChangeTargetLinksOnFCSpy:fnChangeTargetLinksOnFCSpy,
    fnAdjustButtonLocation:fnAdjustButtonLocation,
    fnFilterViewMore:fnFilterViewMore,
    fnFixDIVsSizes:fnFixDIVsSizes,
    fnDisableCode:fnDisableCode
  }
...
function fnShowCEP(IDProd){
  if(FC$.TypeFrt==3 || FC$.TypeFrt==4){
    ...
    sCEP+="        <img src='"+ FC$.PathImg +"icon-arrow-cep.svg"+ "' height='50px' width='50px' alt='Simular frete' id='idCEPButton' class='FieldCEPBtn' onclick='fnGetShippingValuesProd("+ IDProd +")'>";
    sCEP+="      </div>";
    sCEP+="    </div>";
    ...
    sCEP+="  </div>";
    if(FC$.TypeFrt==4)sCEP+="<div class='FreightTxtOnlyBR'><img src='"+FC$.PathImg+"icexclamation.svg"+ "'>"+ rk("details-zip-code-simulation-brazil") +"</div>";
    sCEP+="</div>";
...
function fnFooter(){
  ...
  FCLib$.onReady(function(){
    ...
    sF$.fnSetHeightWidthLoading();
    sF$.fnDisableCode();

    if(FC$.Page=="Products"){
      window.addEventListener("load",sF$.fnLoadSmartModalProductList);
      window.addEventListener('load',sF$.fnFilterViewMore);
...
function fnShowCEPGrid(IDProd,IDProdSimul){
  if(FC$.TypeFrt==3){
    ...
    sCEP+="        <div class='FieldCEP FieldCEPQty'><label>"+ rk("details-zip-code-qty") +"</label><input type='number' id='idQtdZip"+ IDProd +"' value='1' maxlength='4' aria-label='qtd'></div>";
    sCEP+="        <div class='FieldCEP FieldCEPNum'><input type='text' placeholder='CEP' id='idZip"+ IDProd +"' value='"+ sNumCEP +"' maxlength='9' aria-label='zip code'></div>";
    sCEP+="        <img src='"+ FC$.PathImg +"icon-arrow-cep.svg"+ "' height='50px' width='50px' id='idCEPButton' alt='Simular frete' class='FieldCEPBtn' onclick='fnGetShippingValuesProdGrid("+ IDProd +")'>";
    sCEP+="      </div>";
    sCEP+="    </div>";
    sCEP+="    <div id='idDivImgLoadingCEPFC'><img src='"+ FC$.PathImg +"loadingcep.gif"+ "' alt=' ' vspace=3 style='display:none;' id=ImgLoadingCEP></div>";
    sCEP+="    <div id='idShippingValues"+ IDProd +"'></div></div>";
    sCEP+="  </div>";
    sCEP+="</div>";
    var oShowCEP=document.getElementById("ShowCEP"+IDProd);
...
function fnNewsVoiceSearch(){
  if(sF$.fnGetConfig("News_Voice_Search")){
    if(FC$.Page=="News"){
      ...
      newDivSpeech.innerHTML="<div id='fc-icon-voice-news'><img src='"+ FC$.PathImg +"icon-speech-news.svg"+ "' class='header-speech-icon' id='voiceSearch' width='30' height='30' alt='Voice'></div>";
...
var SaveCart$=(function(){
  ...
  function fnShowSaveCartDesign(oJSON,iErr,sMsg){
    if(oJSON.msg!=undefined){
      ...
      if(ValProdsOri!=ValProds && !oSaveCartDesign){ /* if the value of the products without promotion is different from the value that is in the basket displays value saved */
        ...
        var oInsert=document.getElementById("TabItens");
        if(oInsert){
          var oNewElement=document.createElement("div");
...
var Cart$=(function(){
  ...
  function fnShowCartDesign(oJSON,iErr,sMsg){
    ...
    if(oJSON.msg!=undefined){
      ...
      var giftConfig=oJSON.giftConfig;
      var giftValue=oJSON.giftValue;
      iItensCesta=TotalQtyProdCart;
      ...
      for(i=0;i<iQtdProdsXML;i++){
        ...
        sProdAtual+="        <img title='Remover item da cesta' src='"+ FC$.PathImg +"delete_off.svg"+ "' onmouseover='this.src=FC$.PathImg+\"delete.svg\"' onmouseout='this.src=FC$.PathImg+\"delete_off.svg\"' width=16 onclick='Cart$.fnRemoveProd("+ idProdPed +");'>";
        sProdAtual+="      </div>";
        sProdAtual+="    </div>";        
        ...
        sProdAtual+="    </div>";
        if(giftConfig>=4){
          sProdAtual+="    <div class='CartDesign-product-info-desc' style='text-align:initial'>";
          sProdAtual+="     <input id=Gift"+ idProdPed +" onchange=\"Cart$.fnChangeGift(this,"+idProdPed+","+qtyProdCart+")\" type=checkbox value=True "+ (wrap?" checked":"") +">&nbsp;<label for=Gift"+ idProdPed +">"+ rk("side-cart-shopping-gift-wrap") +" ("+currencyProdCart+" "+giftValue+")</label>";
          sProdAtual+="    </div>";
        }
      ...
      if(sProdutosNaCesta!=""){
        ...
        if(totalWrapValue>0){
          sFinalCart+="<tr>";
          sFinalCart+="<td colspan=3 align=right class=TotItProdCart><div class=DescProdCartGift>"+ rk("side-cart-shopping-gift") +":</td>";
          sFinalCart+="<td colspan=2 align=right class=TotItProdCartValor>&nbsp;&nbsp;"+ FCLib$.formatMoney(totalWrapValue,currencyProdCart) +"</div></td>";
          sFinalCart+="</tr>";
        }
        /* Displays Subtotal */
    ...
    sTopo+="<img src='"+ FC$.PathImg +"icon-bot-close-cart.svg"+ "' alt='close cart' onclick='Cart$.fnCloseCartDesign();' style='cursor:pointer;'>";  
    sTopo+="</div>";
    sTopo+="</div>";    
  ...
  function fnChangeGift(Checkbox,idProdPed,iQtdSolic){
    fnInsertLoading(idProdPed);
    AjaxExecFC(FCLib$.uk("url-recalculate"),"q"+ idProdPed +"="+iQtdSolic+"&e"+idProdPed+"="+Checkbox.checked,false,fnShowCartCheckout,0,""); 
  } 
  ...
  function fnChangeQtdProdExec(idProdPed,iQtdSolic){
    var sGift="";
    var oGiftProd=document.getElementById("Gift"+idProdPed);
    if(oGiftProd){if(oGiftProd.checked)sGift="&e"+idProdPed+"=true"};
    fnInsertLoading(idProdPed);
